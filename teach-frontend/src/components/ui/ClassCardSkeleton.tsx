...
function ClassCardSkeleton({ count = 6 }: ClassCardSkeletonProps) {
  return (
    <div className="class-catalog-grid class-catalog-grid--loading" aria-hidden="true">
      {Array.from({ length: count }, (_, index) => (
        <div
          key={index}
          className="class-catalog-card class-catalog-card--skeleton"
        >
          ...
          <div className="skeleton class-catalog-skeleton-cta" />
        </div>
      ))}
    </div>
  )
