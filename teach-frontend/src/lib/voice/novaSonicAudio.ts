...
/** Build the ws:// or wss:// URL for a learning session's spoken viva. */
export function buildVivaVoiceSocketUrl(sessionId: string): string {
  const configuredBase = import.meta.env.VITE_API_BASE_URL ?? ''
  const httpBase = configuredBase !== '' ? configuredBase : window.location.origin
  const url = new URL(`/api/v1/learning-sessions/${sessionId}/viva/voice`, httpBase)
