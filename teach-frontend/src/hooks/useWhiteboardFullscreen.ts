...
export type WhiteboardFullscreenMode = 'normal' | 'native' | 'pseudo'

export function useWhiteboardFullscreen(targetRef: RefObject<HTMLElement | null>) {
  const [mode, setMode] = useState<WhiteboardFullscreenMode>('normal')
  ...
  useEffect(() => {
    document.documentElement.classList.toggle('live-session-fullscreen', mode !== 'normal')
    return () => {
      document.documentElement.classList.remove('live-session-fullscreen')
    }
  }, [mode])
