...
class NovaSonicSession:
    ...
    def _build_client(self) -> Any:
        ...
        # SDK is absent (e.g. running on a Python 3.11 venv).
        try:
        ...
        # Resolve through boto3 rather than reading keys out of settings, so this
        # picks up whatever the rest of the app uses: env vars, a shared profile or
        # an instance role. Matches bedrock_runtime_client.has_aws_credentials().
        import boto3
        ...
        except Exception as error:  # noqa: BLE001 - treated as "no credentials"
            logger.warning("Could not resolve AWS credentials via boto3: %s", error)

        if frozen is None or frozen.access_key is None:
            raise ValidationException(
                "AWS credentials are not configured, so the voice viva cannot start. "
                "Set AWS_ACCESS_KEY_ID / AWS_SECRET_ACCESS_KEY (and AWS_SESSION_TOKEN "
                "for temporary credentials) in the environment."
            )

        identity = AWSCredentialsIdentity(
            access_key_id=frozen.access_key,
            secret_access_key=frozen.secret_key,
            session_token=frozen.token or None,
        )
        config = Config(
